...
## 사본으로 낼 때 가릴 자리 (blind)

제안서 한 벌을 두 가지로 낸다. 발주처에 내는 **원본**, 실적 소개나 외부 공유에 쓰는 **사본**.
사본에서는 고객사가 준 수치, 사람 이름, 우리 사명과 로고가 나가면 안 된다.
장표를 두 벌 만들면 한쪽만 고쳐지는 일이 반드시 생기므로, 한 벌만 두고 **어디를 가릴지만** 따로 적는다.

```
blind.marks = { "n.2.0.1.0.1.0.0": { reason: "자문위원 실명", by: "ai" } }
```

`patches` 에 섞지 않은 까닭 — `patches` 는 "어떻게 보이는가" 이고 블라인드는
"어디까지 내보내도 되는가" 라는 다른 축이다. 서식에 섞으면 **서식을 지울 때 블라인드가 함께
지워진다.** 지워진 줄 모르고 사본을 내면 그때는 되돌릴 수 없다.

| 어디 | 무엇이 보이는가 |
|---|---|
| 편집기 화면 | 원문 + 형광펜 자국. 가릴 자리를 안 보면서 칠할 수는 없다 |
| 원본 내보내기 | 전부 그대로. 자국도 표시 속성도 나가지 않는다 |
| 사본 내보내기 | 칠한 자리 · 사명 · 로고가 `*****` |

사명과 로고는 문서가 아니라 **규칙**이다(`contract/blind.ts` 의 `IDENTITY_PATTERNS`·`LOGO_SELECTORS`).
53장 모든 꼬리말에 로고가 있고 사명은 아무 데나 나오는데, 한 번 빠뜨리면 사본이 사본이 아니게 된다.
